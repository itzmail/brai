...
use brai::config::migration::{self, CURRENT_SCHEMA_VERSION, V1Compat};

fn migrate(toml_str: &str) -> brai::config::Config {
...
fn exhaustive_walk_no_props_lost() {
    use brai::config::{Config, ModelProviderConfig};
...
"#;
    ...
    let known_keys = {
        let mut keys: Vec<String> = toml::to_string(&brai::config::Config::default())
            .ok()

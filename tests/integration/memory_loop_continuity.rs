...
use brai::memory::sqlite::SqliteMemory;
use brai::memory::traits::{Memory, MemoryCategory};
use brai::providers::ToolCall;
...
async fn compressor_with_memory_saves_summary() {
    use brai::agent::context_compressor::{ContextCompressionConfig, ContextCompressor};
...
async fn consolidation_extracts_facts_to_memory() {
    ...
    let result = brai::memory::consolidation::consolidate_turn(
        &provider,
        "test-model",
        mem.as_ref(),
...
async fn memory_survives_rapid_consolidation() {
    ...
    for i in 0..10 {
        ...
        let _ = brai::memory::consolidation::consolidate_turn(
            &provider,
            "test-model",
            mem.as_ref(),
...
async fn session_backend_persists_messages() {
    use brai::channels::session_backend::SessionBackend;
    use brai::channels::session_sqlite::SqliteSessionBackend;
    use brai::providers::traits::ChatMessage;

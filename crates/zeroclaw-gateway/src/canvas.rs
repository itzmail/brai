...
pub async fn handle_canvas_post(
    ...
    if !brai_runtime::tools::ALLOWED_CONTENT_TYPES.contains(&content_type) {
        return (
            ...
            Json(serde_json::json!({
                "error": format!(
                    ...
                    content_type,
                    brai_runtime::tools::ALLOWED_CONTENT_TYPES
                )
            })),
        )
            .into_response();
    ...
    if body.content.len() > brai_runtime::tools::MAX_CONTENT_SIZE {
        return (
            StatusCode::PAYLOAD_TOO_LARGE,
            Json(serde_json::json!({
                "error": format!(
                    "Content exceeds maximum size of {} bytes",
                    brai_runtime::tools::MAX_CONTENT_SIZE
                )
            })),
        )
            .into_response();

...
///
/// Resolution order: `BRAI_WORKSPACE` env → `active_workspace.toml` marker → `~/.brai/config.toml`.
...
pub struct Config {
    ...
    ///
    /// If omitted or empty, the locale is auto-detected from `BRAI_LOCALE`,
...
pub struct WorkspaceConfig {
    /// Turn on multi-workspace profiles — each named engagement gets its own memory, secrets, and audit directories so work for one client/project never bleeds into another. Leave off for single-workspace mode where everything lives under `~/.brai/workspace`.
    #[serde(default)]
    pub enabled: bool,
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "brai_memories".
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `BRAI_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
    #[secret]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `BRAI_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
    #[secret]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
...
pub struct NevisRoleMappingConfig {
    ...
    #[serde(default)]
    pub brai_permissions: Vec<String>,
...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            workspace_dir: brai_dir.join("workspace"),
            config_path: brai_dir.join("config.toml"),
...
fn default_config_dir() -> Result<PathBuf> {
    if let Ok(custom) = std::env::var("BRAI_CONFIG_DIR") {
    ...
    if let Ok(home) = std::env::var("HOME")
        && !home.is_empty()
    {
        return Ok(PathBuf::from(home).join(".brai"));
    }
...
/// `BRAI_CONFIG_DIR` > `BRAI_WORKSPACE` > active workspace marker > defaults.
pub async fn resolve_runtime_dirs_for_onboarding() -> Result<(PathBuf, PathBuf)> {
    let (default_brai_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
    let (config_dir, workspace_dir, _) =
        resolve_runtime_config_dirs(&default_brai_dir, &default_workspace_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "BRAI_CONFIG_DIR",
            Self::EnvWorkspace => "BRAI_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    default_brai_dir: &Path,
    default_workspace_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    if let Ok(custom_config_dir) = std::env::var("BRAI_CONFIG_DIR") {
        let custom_config_dir = custom_config_dir.trim();
        if !custom_config_dir.is_empty() {
            let brai_dir = expand_tilde_path(custom_config_dir);
            return Ok((
                brai_dir.clone(),
                brai_dir.join("workspace"),
                ConfigResolutionSource::EnvConfigDir,
            ));
        }
    }

    if let Ok(custom_workspace) = std::env::var("BRAI_WORKSPACE")
        && !custom_workspace.is_empty()
    {
        let expanded = expand_tilde_path(&custom_workspace);
        let (brai_dir, workspace_dir) = resolve_config_dir_for_workspace(&expanded);
        return Ok((
            brai_dir,
            workspace_dir,
    ...
    if let Some((brai_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_brai_dir).await?
    {
        return Ok((
            brai_dir,
            workspace_dir,
...
fn has_ollama_cloud_credential(config_api_key: Option<&str>) -> bool {
    ...
    ["OLLAMA_API_KEY", "BRAI_API_KEY", "API_KEY"]
        .iter()
        .any(|name| {
            std::env::var(name)
                .ok()
...
/// Parse the `BRAI_EXTRA_HEADERS` environment variable value.
///
...
pub fn parse_extra_headers_env(raw: &str) -> Vec<(String, String)> {
    ...
    for entry in raw.split(',') {
        ...
        if let Some((key, value)) = entry.split_once(':') {
            ...
            if key.is_empty() {
                tracing::warn!("Ignoring extra header with empty name in BRAI_EXTRA_HEADERS");
                continue;
            }
...
impl Config {
    ...
    pub async fn load_or_init() -> Result<Self> {
        ...
        let (brai_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_brai_dir, &default_workspace_dir).await?;

        let config_path = brai_dir.join("config.toml");

        fs::create_dir_all(&brai_dir)
            .await
            .with_context(|| config_dir_creation_error(&brai_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::secrets::SecretStore::new(&brai_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: BRAI_API_KEY or API_KEY (generic)
        if let Ok(key) = std::env::var("BRAI_API_KEY").or_else(|_| std::env::var("API_KEY"))
            && !key.is_empty()
        {
        ...
        // 1) BRAI_PROVIDER always wins when set.
        // 2) BRAI_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        // 3) Legacy PROVIDER is honored only when config still uses default provider.
        if let Ok(provider) = std::env::var("BRAI_PROVIDER")
            && !provider.is_empty()
        {
            self.providers.fallback = Some(provider);
        } else if let Ok(provider) =
            std::env::var("BRAI_MODEL_PROVIDER").or_else(|_| std::env::var("MODEL_PROVIDER"))
            && !provider.is_empty()
        {
            self.providers.fallback = Some(provider);
        ...
        // Model: BRAI_MODEL or MODEL
        if let Ok(model) = std::env::var("BRAI_MODEL").or_else(|_| std::env::var("MODEL"))
            && !model.is_empty()
        {
            self.ensure_fallback_provider().model = Some(model);
        }

        // Provider HTTP timeout: BRAI_PROVIDER_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("BRAI_PROVIDER_TIMEOUT_SECS")
        ...
        // Extra provider headers: BRAI_EXTRA_HEADERS
        // Format: "Key:Value,Key2:Value2"
        // Env var headers override config file headers with the same name.
        if let Ok(raw) = std::env::var("BRAI_EXTRA_HEADERS") {
        ...
        // Workspace directory: BRAI_WORKSPACE
        if let Ok(workspace) = std::env::var("BRAI_WORKSPACE")
            && !workspace.is_empty()
        {
        ...
        // Open-skills opt-in flag: BRAI_OPEN_SKILLS_ENABLED
        if let Ok(flag) = std::env::var("BRAI_OPEN_SKILLS_ENABLED")
            && !flag.trim().is_empty()
        {
        ...
        // Open-skills directory override: BRAI_OPEN_SKILLS_DIR
        if let Ok(path) = std::env::var("BRAI_OPEN_SKILLS_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Skills script-file audit override: BRAI_SKILLS_ALLOW_SCRIPTS
        if let Ok(flag) = std::env::var("BRAI_SKILLS_ALLOW_SCRIPTS")
            && !flag.trim().is_empty()
        {
            match flag.trim().to_ascii_lowercase().as_str() {
                ...
                _ => tracing::warn!(
                    "Ignoring invalid BRAI_SKILLS_ALLOW_SCRIPTS (valid: 1|0|true|false|yes|no|on|off)"
                ),
            }
        }

        // Skills prompt mode override: BRAI_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("BRAI_SKILLS_PROMPT_MODE")
            && !mode.trim().is_empty()
        {
            if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                ...
                tracing::warn!(
                    "Ignoring invalid BRAI_SKILLS_PROMPT_MODE (valid: full|compact)"
                );
            }
        }

        // Gateway port: BRAI_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("BRAI_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
            && let Ok(port) = port_str.parse::<u16>()
        {
            self.gateway.port = port;
        }

        // Gateway host: BRAI_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("BRAI_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
            && !host.is_empty()
        {
            self.gateway.host = host;
        }

        // Allow public bind: BRAI_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("BRAI_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Require pairing: BRAI_REQUIRE_PAIRING
        if let Ok(val) = std::env::var("BRAI_REQUIRE_PAIRING") {
            self.gateway.require_pairing = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Web dist dir: BRAI_WEB_DIST_DIR
        if let Ok(path) = std::env::var("BRAI_WEB_DIST_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Temperature: BRAI_TEMPERATURE
        if let Ok(temp_str) = std::env::var("BRAI_TEMPERATURE") {
            match temp_str.parse::<f64>() {
                ...
                Ok(temp) => {
                    tracing::warn!(
                        "Ignoring BRAI_TEMPERATURE={temp}: \
                ...
                Err(_) => {
                    tracing::warn!(
                        "Ignoring BRAI_TEMPERATURE={temp_str:?}: not a valid number"
                    );
                }
            }
        }

        // Reasoning override: BRAI_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) = std::env::var("BRAI_REASONING_ENABLED")
        ...
        if let Ok(raw) = std::env::var("BRAI_REASONING_EFFORT")
            .or_else(|_| std::env::var("REASONING_EFFORT"))
            .or_else(|_| std::env::var("BRAI_CODEX_REASONING_EFFORT"))
        {
        ...
        // Web search enabled: BRAI_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("BRAI_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: BRAI_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("BRAI_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: BRAI_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("BRAI_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // Tavily API key: BRAI_TAVILY_API_KEY or TAVILY_API_KEY
        if let Ok(api_key) =
            std::env::var("BRAI_TAVILY_API_KEY").or_else(|_| std::env::var("TAVILY_API_KEY"))
        {
            let api_key = api_key.trim();
            if !api_key.is_empty() {
        ...
        // SearXNG instance URL: BRAI_SEARXNG_INSTANCE_URL or SEARXNG_INSTANCE_URL
        if let Ok(instance_url) = std::env::var("BRAI_SEARXNG_INSTANCE_URL")
        ...
        // Web search max results: BRAI_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("BRAI_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: BRAI_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("BRAI_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): BRAI_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("BRAI_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): BRAI_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("BRAI_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: BRAI_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("BRAI_STORAGE_CONNECT_TIMEOUT_SECS")
            && let Ok(timeout_secs) = timeout_secs.parse::<u64>()
            && timeout_secs > 0
        {
            self.storage.provider.config.connect_timeout_secs = Some(timeout_secs);
        }
        // Proxy enabled flag: BRAI_PROXY_ENABLED
        let explicit_proxy_enabled = std::env::var("BRAI_PROXY_ENABLED")
            .ok()
            .as_deref()
        ...
        // Proxy URLs: BRAI_* wins, then generic *PROXY vars.
        let mut proxy_url_overridden = false;
        if let Ok(proxy_url) =
            std::env::var("BRAI_HTTP_PROXY").or_else(|_| std::env::var("HTTP_PROXY"))
        {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("BRAI_HTTPS_PROXY").or_else(|_| std::env::var("HTTPS_PROXY"))
        {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("BRAI_ALL_PROXY").or_else(|_| std::env::var("ALL_PROXY"))
        {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(no_proxy) =
            std::env::var("BRAI_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
        ...
        if let Ok(scope_raw) = std::env::var("BRAI_PROXY_SCOPE") {
            if let Some(scope) = parse_proxy_scope(&scope_raw) {
                self.proxy.scope = scope;
            } else {
                tracing::warn!(
                    scope = %scope_raw,
                    "Ignoring invalid BRAI_PROXY_SCOPE (valid: environment|zeroclaw|services)"
                );
            }
        }

        if let Ok(services_raw) = std::env::var("BRAI_PROXY_SERVICES") {
    ...
    async fn resolve_config_path_for_save(&self) -> Result<PathBuf> {
        ...
        let (default_brai_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
        let (brai_dir, _workspace_dir, source) =
            resolve_runtime_config_dirs(&default_brai_dir, &default_workspace_dir).await?;
        let file_name = self
            .config_path
            .file_name()
        ...
        let resolved = brai_dir.join(file_name);
        tracing::warn!(
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let brai_dir = config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::secrets::SecretStore::new(brai_dir, self.secrets.encrypt);
...
mod tests {
    ...
    async fn expand_tilde_path_expands_tilde_when_home_set() {
        ...
        let path = expand_tilde_path("~/.brai");
...
"#;
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("brai_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("brai_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.brai/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
            mention_only: false,
...
"#;
    ...
    async fn env_override_api_key() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_API_KEY", "sk-test-env-key") };
        config.apply_env_overrides();
        assert_eq!(
            config
                .providers
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_API_KEY") };
    }
    ...
    async fn env_override_provider() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_PROVIDER", "anthropic") };
        config.apply_env_overrides();
        assert_eq!(config.providers.fallback.as_deref(), Some("anthropic"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_PROVIDER") };
    }
    ...
    async fn env_override_model_provider_alias() {
        ...
        unsafe { std::env::remove_var("BRAI_PROVIDER") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_MODEL_PROVIDER", "openai-codex") };
        config.apply_env_overrides();
        assert_eq!(config.providers.fallback.as_deref(), Some("openai-codex"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_MODEL_PROVIDER") };
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_OPEN_SKILLS_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_OPEN_SKILLS_DIR", "/tmp/open-skills") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_SKILLS_ALLOW_SCRIPTS", "yes") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_SKILLS_PROMPT_MODE", "compact") };
        config.apply_env_overrides();
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_OPEN_SKILLS_ENABLED") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_OPEN_SKILLS_DIR") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_SKILLS_ALLOW_SCRIPTS") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_SKILLS_PROMPT_MODE") };
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_OPEN_SKILLS_ENABLED", "maybe") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_SKILLS_ALLOW_SCRIPTS", "maybe") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_SKILLS_PROMPT_MODE", "invalid") };
        config.apply_env_overrides();
    ...
    async fn env_override_zero_claw_provider_overrides_non_default_provider() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_PROVIDER", "openrouter") };
        // SAFETY: test-only, single-threaded test runner.
    ...
    async fn env_override_model() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_MODEL", "gpt-4o") };
        config.apply_env_overrides();
        assert_eq!(
            config
                .providers
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_MODEL") };
    }
...
"#;
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("brai_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let resolved_config_path = temp_home.join(".brai").join("config.toml");

        let original_home = std::env::var("HOME").ok();
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("HOME", &temp_home) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_WORKSPACE", &workspace_dir) };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_WORKSPACE") };
    ...
    async fn env_override_workspace() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_WORKSPACE", "/custom/workspace") };
        config.apply_env_overrides();
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_CONFIG_DIR", &explicit_config_dir) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_WORKSPACE") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_CONFIG_DIR") };
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    #[test]
    async fn default_path_under_config_dir_respects_brai_config_dir() {
        let _env_guard = env_override_lock().await;
        let custom_dir = std::env::temp_dir().join("zeroclaw-test-profile");
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_CONFIG_DIR", &custom_dir) };
        ...
        assert_eq!(
            ...
            "expected path under BRAI_CONFIG_DIR, got: {result}"
        );
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("brai_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_path = temp_home.join(".brai").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".brai");
...
"#,
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("brai_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".brai");
    ...
    async fn load_or_init_uses_persisted_active_workspace_marker() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("brai_test_home_{}", uuid::Uuid::new_v4()));
        let temp_default_dir = temp_home.join(".brai");
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        unsafe { std::env::set_var("HOME", &temp_home) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_WORKSPACE", &env_workspace_dir) };
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        let temp_home =
            std::env::temp_dir().join(format!("brai_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".brai");
...
"#,
    ...
    async fn env_override_empty_values_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_PROVIDER", "") };
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_GATEWAY_PORT", "8080") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_GATEWAY_PORT") };
    }
    ...
    async fn env_override_gateway_host() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_GATEWAY_HOST", "0.0.0.0") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_GATEWAY_HOST") };
    }
    ...
    async fn env_override_require_pairing() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_REQUIRE_PAIRING", "false") };
        config.apply_env_overrides();
        assert!(!config.gateway.require_pairing);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_REQUIRE_PAIRING", "true") };
        config.apply_env_overrides();
        assert!(config.gateway.require_pairing);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_REQUIRE_PAIRING") };
    }
    ...
    async fn env_override_temperature() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_TEMPERATURE", "0.5") };
        config.apply_env_overrides();
        assert!(
            (config
                .providers
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_TEMPERATURE") };
    }
    ...
    async fn env_override_temperature_out_of_range_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_TEMPERATURE", "3.0") };
        config.apply_env_overrides();
        assert!(
            (config
                .providers
    ...
    async fn env_override_reasoning_enabled() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_REASONING_ENABLED", "false") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_REASONING_ENABLED", "true") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_REASONING_ENABLED") };
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_REASONING_ENABLED", "maybe") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_reasoning_effort() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_REASONING_EFFORT", "HIGH") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("high"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_REASONING_EFFORT") };
    }
    ...
    async fn env_override_reasoning_effort_legacy_codex_env() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_CODEX_REASONING_EFFORT", "minimal") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("minimal"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_CODEX_REASONING_EFFORT") };
    }
    ...
    async fn env_override_storage_provider_config() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_STORAGE_PROVIDER", "qdrant") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_STORAGE_DB_URL", "http://localhost:6333") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_STORAGE_CONNECT_TIMEOUT_SECS", "15") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_STORAGE_PROVIDER") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_STORAGE_DB_URL") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("BRAI_STORAGE_CONNECT_TIMEOUT_SECS") };
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_PROXY_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_HTTP_PROXY", "http://127.0.0.1:7890") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe {
            std::env::set_var(
                "BRAI_PROXY_SERVICES",
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_PROXY_SCOPE", "services") };
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        unsafe { std::env::set_var("BRAI_PROXY_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_PROXY_SCOPE", "environment") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_HTTP_PROXY", "http://127.0.0.1:7890") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_HTTPS_PROXY", "http://127.0.0.1:7891") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("BRAI_NO_PROXY", "localhost,127.0.0.1") };
...
[security.estop]
enabled = true
state_file = "~/.brai/estop-state.json"
require_otp_to_resume = true
"#,
        );
...
config_path = "/zeroclaw-data/.brai/config.toml"
api_key = ""
default_provider = "openrouter"

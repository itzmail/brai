...
// in `~/.brai/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.brai/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(brai_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: brai_dir.join(".secret_key"),
            enabled,
        }
    }

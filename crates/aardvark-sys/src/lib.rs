...
//!
//! 1. `BRAI_AARDVARK_LIB` environment variable (full path to `aardvark.so`)
...
fn lib() -> Option<&'static Library> {
    AARDVARK_LIB
        .get_or_init(|| {
            ...
            if !tried_any {
                eprintln!("[aardvark-sys] no library candidates found; set BRAI_AARDVARK_LIB or place aardvark.so next to the binary");
            }
            None
        })
        .as_ref()
...
pub enum AardvarkError {
    ...
    #[error("aardvark.so not found — set BRAI_AARDVARK_LIB or place it next to the binary")]
    LibraryNotFound,
}

//! Plugin manifest loader — scans `~/.brai/tools/` at startup.
//!
...
/// Scan `~/.brai/tools/` and return all valid plugins.
///
...
"#;
    ...
    fn scan_plugin_dir_skips_broken_manifests_without_panicking() {
        ...
        // always uses ~/.brai/tools), but we can verify load_one_plugin
    ...
    #[test]
    fn plugin_tools_dir_returns_path_ending_in_brai_tools() {
        let path = plugin_tools_dir().expect("should resolve");
        let display = path.to_string_lossy();
        let expected = std::path::Path::new(".brai").join("tools");
